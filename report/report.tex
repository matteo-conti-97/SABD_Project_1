...
Per prima cosa è stato fatto il drop delle colonne inutili per la query, che sono "serial\_number", "model" e "power\_on\_hours". Successivamente è stato fatto il group by per "date" e "vault\_id" e la somma dei fallimenti (essendo i fallimenti rappresentati dai valori 0 e 1 la loro somma rappresenta il numero di fallimenti). Infine è stato effettuato un filtraggio che seleziona solo i vault che hanno subito un numero di fallimenti >= 2 e <= 4.
